...
# ///
"""
Qwen3-ASR Hebrew Fine-tuning - Fixed Version

This version directly accesses audio arrays from the dataset
instead of trying to extract file paths, avoiding torchcodec issues.
"""
...
def save_audio_and_create_jsonl(dataset, output_dir, split_name):
    ...
    FIXED: Directly accesses decoded audio arrays instead of file paths.
    """
    ...
    with open(jsonl_path, 'w', encoding='utf-8') as f:
        for idx in range(len(dataset)):
            try:
                # Access the example directly - this will decode the audio
                example = dataset[idx]
                text = example["text"]
                ...
                # Get audio data - this is already decoded by datasets library
...
def main():
    ...
    print("=" * 60)
    print("Qwen3-ASR Hebrew Fine-tuning (Fixed)")
    print("=" * 60)
